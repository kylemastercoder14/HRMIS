...
const MyEvaluationPage = ({ params }: { params: { evaluationId: string } }) => {
  ...
  useEffect(() => {
    const fetchEvaluationData = async () => {
      try {
        const response = await getEvaluationById(params.evaluationId);
  ...
  return (
    <>
      ...
      <Card className="shadow-xl -mt-20 w-[93%] mx-auto relative z-50">
        ...
        <Separator />
        <CardContent>
          {currentStep === 1 && <Step1Form evaluationData={evaluationData} nextStep={nextStep} />}
          {currentStep === 2 && (
            <Step2Form evaluationData={evaluationData} nextStep={nextStep} prevStep={prevStep} />
          )}
          {currentStep === 3 && (
            <Step3Form evaluationData={evaluationData} nextStep={nextStep} prevStep={prevStep} />
          )}
          {currentStep === 4 && (
            <Step4Form evaluationData={evaluationData} nextStep={nextStep} prevStep={prevStep} />
          )}
          {currentStep === 5 && (
            <Step5Form evaluationData={evaluationData} nextStep={nextStep} prevStep={prevStep} />
          )}
          {currentStep === 6 && <Step6Form evaluationData={evaluationData} prevStep={prevStep} />}
        </CardContent>
      </Card>
    </>
  );

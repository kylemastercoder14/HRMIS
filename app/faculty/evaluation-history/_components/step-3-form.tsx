...
const Step3Form = ({
  ...
  useEffect(() => {
    const subscription = form.watch((value) => {
      localStorage.setItem("step3DataUpdate", JSON.stringify(value));
    });
  ...
  useEffect(() => {
    if (evaluationData) {
      form.reset({
        mastery: evaluationData?.mastery || "",
        draws: evaluationData?.draws || "",
        practical: evaluationData?.practical || "",
        relevance: evaluationData?.relevance || "",
        awareness: evaluationData?.awareness || "",
      });
    }
  }, [evaluationData, form]);
  ...
  const onSubmit = async (values: z.infer<typeof Step3Schema>) => {
    setIsPending(true);
    try {
      localStorage.setItem("step3DataUpdate", JSON.stringify(values));
      nextStep();
    } finally {
      setIsPending(false);
    }

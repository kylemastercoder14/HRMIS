...
import { Step1Schema, Step2Schema, Step3Schema, Step4Schema } from "@/lib/validators";
...
const Step4Form = ({
  ...
  const savedData =
    typeof window !== "undefined" ? localStorage.getItem("step4Data") : null;
  ...
  useEffect(() => {
    const subscription = form.watch((value) => {
      localStorage.setItem("step4Data", JSON.stringify(value));
    });
  ...
  const onSubmit = async (values: z.infer<typeof Step4Schema>) => {
    setIsPending(true);
    try {
      localStorage.setItem("step4Data", JSON.stringify(values));
      nextStep();
    } finally {
      setIsPending(false);
    }

...
import { Step1Schema, Step2Schema, Step3Schema, Step4Schema, Step5Schema } from "@/lib/validators";
...
const Step5Form = ({
  ...
  const savedData =
    typeof window !== "undefined" ? localStorage.getItem("step5Data") : null;
  ...
  useEffect(() => {
    const subscription = form.watch((value) => {
      localStorage.setItem("step5Data", JSON.stringify(value));
    });
  ...
  const onSubmit = async (values: z.infer<typeof Step5Schema>) => {
    setIsPending(true);
    try {
      localStorage.setItem("step5Data", JSON.stringify(values));
      nextStep();
    } finally {
      setIsPending(false);
    }
